...
async function rows(t){if(!sb)return fb[t] instanceof Array?fb[t]:[fb[t]];let r=await sb.from(t).select('*').order('created_at',{ascending:true});return r.data?.length?r.data:(fb[t] instanceof Array?fb[t]:[fb[t]])}
function paintLive(live){document.querySelector('#liveHeading').textContent=live.enabled?'🔴 LIVE NOW':'⚫ OFFLINE';document.querySelector('#liveTitle').textContent=live.title;document.querySelector('#liveDesc').textContent=live.enabled?live.description:'Currently offline — follow for the next stream.';document.querySelector('#liveUrl').href=live.url;document.querySelector('#liveUrl').textContent=live.enabled?'OPEN STREAM':'FOLLOW ON TWITCH'}
async function refreshLive(){if(!sb)return paintLive(fb.live);let r=await sb.from('live_status').select('*').order('updated_at',{ascending:false}).limit(1);if(r.data?.[0])paintLive(r.data[0])}

function esc(v=''){return String(v).replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]))}
function isoSeconds(iso='PT0S'){let m=iso.match(/PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?/);if(!m)return 0;return (Number(m[1]||0)*3600)+(Number(m[2]||0)*60)+Number(m[3]||0)}
function durationLabel(seconds){let h=Math.floor(seconds/3600),m=Math.floor((seconds%3600)/60),s=seconds%60;return h?`${h}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`:`${m}:${String(s).padStart(2,'0')}`}
function viewsLabel(n){n=Number(n||0);if(n>=1000000)return `${(n/1000000).toFixed(n>=10000000?0:1)}M views`;if(n>=1000)return `${(n/1000).toFixed(n>=100000?0:1)}K views`;return `${n} views`}
...
async function loadYouTube(manualClips=[]){
 ...
 try{
  ...
  videoGrid.innerHTML=videos.length?videos.map(v=>videoCard(v,'VIDEO')).join(''):'<article class="video-status">No videos found yet.</article>';
  clipGrid.innerHTML=shorts.length?shorts.map(v=>videoCard(v,'SHORT')).join(''):(manualClips.length?manualClipCards(manualClips):'<article class="video-status">No Shorts or clips found yet.</article>');
